...
# options/memory/003_train_memory_videohdr_youtube.json
# options/transformer/006_pretrain + transformer_videohdr_youtube.json
def main(json_path='options/memory/007_train_dense_memory_videohdr_youtube.json'):
    ...
    for epoch in range(10000000):  # keep running
        for i, train_data in enumerate(train_loader):
            ...
            # H = train_data['H'].clone().detach() 
            # L = train_data['L'].clone().detach()  

            # print(torch.equal(H, L))

            # # 创建一个SummaryWriter对象来写入TensorBoard日志
            # util.mkdir('logs')
            # writer = SummaryWriter(log_dir='logs')


            # for i in range(train_data['H'].size(0)):
            #     writer.add_images("H", H[i], global_step=i)
            #     writer.add_images("L", L[i], global_step=i)
            ...
            # for i in range(train_data['H'].size(0)):
            #     # 如果是phase2，就改成np.uint16
            #     gt = util.tensor2img(train_data['H'][i], np.uint16)  # CHW-RGB to HCW-BGR uint16
            #     lq = util.tensor2img(train_data['L'][i], np.uint8)  # CHW-RGB to HCW-BGR uint8
            #     # print(gt==lq)
            #     cv2.imwrite(f'{save_dir}/{current_step:d}_{i}_gt.png', gt)
            #     cv2.imwrite(f'{save_dir}/{current_step:d}_{i}_lq.png', lq)
            #     print(f'{save_dir}/{current_step:d}_{i}_gt.png')
            ...
            # 6) testing
            # -------------------------------
            if (current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0) or current_step > opt['train']['total_iter']:
                ...
                for data in test_loader:
                    ...
                    model.feed_data(data, need_H=need_GT)
                    ...
                    # save images
                    if opt['val']['save_img']:
                        save_dir = osp.join(opt['path']['images'], str(current_step))
                        util.mkdir(save_dir)
                        cv2.imwrite(f'{save_dir}/{img_name}.png', sr_img)
                    ...
                    print('{:10s} - PSNR: {:.6f} dB'.format(img_name, PSNR))
                ...
                logger.info('<epoch:{:3d}, iter:{:8,d} Average PSNR: {:.6f} dB'.format(epoch, current_step, ave_psnr))
