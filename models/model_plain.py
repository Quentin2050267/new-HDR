...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'charbonnier+perceptual':
            self.G_lossfn = CharbonnierLoss(self.opt_train['G_charbonnier_eps']).to(self.device)
            self.G_lossfn_perceptual = PerceptualLoss(weights=[1,1,1,1,1])
        else:
            raise NotImplementedError('Loss type [{:s}] is not found.'.format(G_lossfn_type))
        self.G_lossfn_weight = self.opt_train['G_lossfn_weight']
        self.pLoss_weight = self.opt_train.get('pLoss_weight', None)
        print(self.pLoss_weight)
    ...
    def optimize_parameters(self, current_step):
        ...
        if self.pLoss_weight:
            p_loss = self.pLoss_weight * self.G_lossfn_perceptual(self.E, self.H)
            P_loss = P_loss + p_loss
            P_loss.backward()
        else:
            G_loss.backward()
