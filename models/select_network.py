...
def define_G(opt):
    ...
    elif net_type == 'rvrtrans':
        from models.network_transformer import RVRTrans as net
        netG = net(upscale=opt_net['upscale'],
                   clip_size=opt_net['clip_size'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
    ...
    elif net_type == 'memtrans':
        from models.transformer_memory.network_memory import MEMTrans as net
        netG = net(upscale=opt_net['upscale'],
                   clip_size=opt_net['clip_size'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
                   ...
                   no_checkpoint_ffn_blocks=opt_net['no_checkpoint_ffn_blocks'],
                   cpu_cache_length=opt_net['cpu_cache_length'],
                   max_mem = opt_net['max_mem'],
                   num_mem_blocks = opt_net['num_mem_blocks']
                   )
        
    elif net_type == 'densememtrans':
        from models.dense_memory.network_dense_mem import MEMDenseTrans as net
        netG = net(upscale=opt_net['upscale'],
                   clip_size=opt_net['clip_size'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
    ...
    elif net_type == 'Pretrain':
        from models.pretrain.network_pretrain import Pretrain as net
        netG = net(clip_size=opt_net['clip_size'],
                   img_size=opt_net['img_size'],
                   window_size=opt_net['window_size'],
                   num_blocks=opt_net['num_blocks'],
                   depths=opt_net['depths'],
...
def define_F(opt, use_bn=False):
    device = torch.device(opt['gpu_ids'][0] if opt['gpu_ids'] else 'cpu')
...
def init_weights(net, init_type='xavier_uniform', init_bn_type='uniform', gain=1):
    ...
    if init_type not in ['default', 'none']:
        ...
        print(init_type, init_bn_type, gain)
